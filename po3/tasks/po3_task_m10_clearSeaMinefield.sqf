...
// =========================================================================================================
	...
	_position = [PO3_TASK__POS,[10,300],1,2,10] call mpsf_fnc_getSafePos;
...
// =========================================================================================================
	...
	for "_i" from 0 to 1000 do {
		_testPos = [ [_position,[0,150],random 360],[8,15],7,2,10] call mpsf_fnc_getSafePos;
		_testPos set [2,0];
	...
	if(_mineTruck) then {
		_shorePos = [_position,[10,300],5,0,1] call mpsf_fnc_getSafePos;
		for "_i" from 1 to (PO3_TASK__DIF*3) do {
			_grp = nil;
			_grp = [_shorePos , (po3_side_3 select 0), "EN_PatrolGroup0", 100 ] call mpsf_fnc_createGroup;
			if !(isNil "_grp") then {
				[ _shorePos, _grp, 100 ] spawn mpsf_fnc_groupDefendPos;
				PO3_TOTAL_UNITS = PO3_TOTAL_UNITS + (units _grp);
				sleep 1;
			};

			_grp = nil;
			_grp = [ _shorePos, (po3_side_3 select 0), format["EN_GroupForce_%1",round random 4], 100 ] call mpsf_fnc_createGroup;
			if !(isNil "_grp") then {
				[ _shorePos, _grp, (50 * _i) ] spawn mpsf_fnc_groupPatrolArea;
				PO3_TOTAL_UNITS = PO3_TOTAL_UNITS + (units _grp);
				sleep 1;
			};
			...
			if(count _class > 0) then {
				_veh = ([ _shorePos,_class call mpsf_fnc_getArrayRandom,random 360,20,(po3_side_3 select 0)] call mpsf_fnc_createVehicle) select 0;
				if !(isNil "_veh") then {
...
// =========================================================================================================
	...
	for "_i" from 1 to _numBoats do {
		...
		if(count _class > 0) then {
			_veh = ([ [_position,[200,500],5,2,10] call mpsf_fnc_getSafePos,_class call mpsf_fnc_getArrayRandom,random 360,10,(po3_side_3 select 0),"EN_Squad_Divers"] call mpsf_fnc_createVehicle) select 0;
			if !(isNil "_veh") then {
				[ position(leader _veh), _veh, 500 + (random 1000)] spawn mpsf_fnc_groupPatrolArea;
				PO3_TOTAL_UNITS = PO3_TOTAL_UNITS + (units _veh);
				PO3_TOTAL_VEHICLES set [ count PO3_TOTAL_VEHICLES, vehicle (leader _veh) ];
			};
			sleep 1;
	...
	for "_i" from 1 to (PO3_TASK__DIF*3) do {
		...
		if(count _class > 0) then {
			...
			if !(isNil "_veh") then {
				[ _position, _veh, 1000 + round(random 2000) ] spawn mpsf_fnc_groupPatrolArea;
				PO3_TOTAL_UNITS = PO3_TOTAL_UNITS + (units _veh);
				PO3_TOTAL_VEHICLES set [ count PO3_TOTAL_VEHICLES, vehicle (leader _veh) ];
			};
		};
	...
	for "_i" from 1 to ceil(round(_b/2) max 1) do {
		_grp = nil;
		_grp = [ _position, (po3_side_3 select 0), "EN_Squad_Divers", 400 ] call mpsf_fnc_createGroup;
		if !(isNil "_grp") then {
			[ _position, _grp, 50 + round(random 350) ] spawn mpsf_fnc_groupPatrolArea;
			{
...
// =========================================================================================================
	...
	[ format["TASK%1",PO3_TASK__IDD],
		...
		(po3_side_1 select 0),
		[format["MRKR%1",PO3_TASK__IDD],PO3_TASK__POS,"mil_unknown","ColorOPFOR"," SeaMines"],
		"assigned",
		(PO3_TASK__POS),
		localize "STR_PO3_M10_TITLE"
