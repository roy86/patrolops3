...
//	_b = 4*(PO3_TASK__DIF*3);
	...
	_nearByBuildings = [_position,_locRadis] call mpsf_fnc_getNearbyBuildings;
	...
	for "_i" from 1 to round(_b*0.75) do {
		...
		_cache setpos ([ [PO3_TASK__POS,[50,(_locRadis*PO3_TASK__DIF)],random 360,false],15,5,0,0.3] call mpsf_fnc_getSafePos);
		if( count _nearByBuildings > 0 ) then {
			_building = _nearByBuildings call mpsf_fnc_getArrayRandom;
		...
		_grp = nil;
		_grp = [ position _cache, (po3_side_3 select 0), "EN_PatrolGroup0", 20 ] call mpsf_fnc_createGroup;
		if !(isNil "_grp") then {
			[ position _cache, _grp, 20] spawn mpsf_fnc_groupDefendPos;
			PO3_TOTAL_UNITS = PO3_TOTAL_UNITS + (units _grp);
			sleep 1;
		};
	};
	_caches call mpsf_fnc_setDamageEH_C4Only;
...
// =========================================================================================================
	...
	[ format["TASK%1",PO3_TASK__IDD],
		...
		format[ localize "STR_PO3_M17_DESCR",PO3_TASK__IDD,(po3_side_3 select 2),_locaname,count _caches,[_cacheType] call mpsf_fnc_getCfgIcon],
		(po3_side_1 select 0),
		[format["MRKR%1",PO3_TASK__IDD],PO3_TASK__POS,"mil_objective","ColorOPFOR",localize "STR_PO3_M17_TITLE"],
		"assigned",
		PO3_TASK__POS,
		localize "STR_PO3_M17_TITLE"
