...
// =========================================================================================================
	...
	_position = [PO3_TASK__POS,[200,2000],5,0,1] call mpsf_fnc_getSafePos;
...
// =========================================================================================================
	...
	_heli = ([ _position,po3_rescue_heliTypes call mpsf_fnc_getArrayRandom,random 360,300] call mpsf_fnc_createVehicle);
	if !(isNil "_heli") then {
...
// =========================================================================================================
	...
	_pilotPosition = [_position,[400,800],1,0,2] call mpsf_fnc_getSafePos;

	_pilotGrp = nil;
	_pilotGrp = [ _pilotPosition, (po3_side_1 select 0), "FR_GroupRescuePilots", 100 ] call mpsf_fnc_createGroup;
	...
	{
		_x setVariable ["mpsf_capture",true,true]; //to be replaced with RESCUE option
	}foreach _rescueUnits;
	_rescueUnitsCount = {alive _x} count _rescueUnits;

	[_rescueUnits] spawn {
		_units = _this select 0;
		_leader = _units call mpsf_fnc_getArrayRandom;
...
// =========================================================================================================
	...
	for "_i" from 0 to (_b/2) do {
		...
		if(random 1 > 0.7) then {
			...
			if(count _class > 0) then {
				_veh = nil;
				_veh = ([ _position,"O_Heli_Light_02_F",random 360,(750 + random 250),(po3_side_3 select 0),[],"FLY"] call mpsf_fnc_createVehicle) select 0;
				if !(isNil "_veh") then {
					[ _position, _veh, 2000] spawn mpsf_fnc_group_patrol;
					PO3_TOTAL_UNITS = PO3_TOTAL_UNITS + (units _veh);
					PO3_TOTAL_VEHICLES set [ count PO3_TOTAL_VEHICLES, vehicle (leader _veh) ];
				};
			};
...
// =========================================================================================================
	...
	[ format["TASK%1",PO3_TASK__IDD],
		...
		format[localize "STR_PO3_M20_DESCR",PO3_TASK__IDD,[_heli] call mpsf_fnc_getCfgText,mapGridPosition _position],
		(po3_side_1 select 0),
		[format["MRKR%1",PO3_TASK__IDD],_position,"mil_unknown","ColorBLUFOR",localize "STR_PO3_M20_MSG_1"],
		"assigned",
		(_position),
...
// =========================================================================================================
	_returnPointLoc = "return_point" call mpsf_fnc_getPos;
	_returnPointLoc_marine = "return_point_marine" call mpsf_fnc_getPos;
