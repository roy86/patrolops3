...
// =========================================================================================================
	...
	_position = [ [PO3_TASK__POS,[0,(1000*PO3_TASK__DIF)],random 360,false],50,10,0,0.3] call mpsf_fnc_getSafePos;
...
//	_b = 2*(PO3_TASK__DIF*3);
	...
	_nearlocations = [_position,3000] call mpsf_fnc_getNearbylocations;
	...
	for "_i" from 0 to _b do {
		...
		if(count _class > 0) then {
			_veh = ([ [_position,50,10,0,0.3] call mpsf_fnc_getSafePos,_class call mpsf_fnc_getArrayRandom,random 360,100,(po3_side_3 select 0)] call mpsf_fnc_createVehicle) select 0;
			if !(isNil "_veh") then {
...

	{
		...
		if(mpsf_debug) then {
			_marker = createMarkerlocal [format["mpsf_temp_ap%1",_x],position _x];
...
// =========================================================================================================
	...
	["PO3",format["Task: %1 %2 Generated at %3","AttackConvoyArmoured",PO3_TASK__IDD,_location],true] spawn mpsf_fnc_log;
