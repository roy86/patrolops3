...
// =========================================================================================================
	...
	[ format["TASK%1",PO3_TASK__IDD],
		...
		(po3_side_1 select 0),
		[format["MRKR%1",PO3_TASK__IDD],_position,"mil_objective","ColorBlufor",localize "STR_PO3_M13_TITLE"],
		"assigned",
		_position,
		localize "STR_PO3_M13_TITLE"
...
// =========================================================================================================
	...
	While{ _percentAlive > 0.15 && count([_position,300,(po3_side_1 select 0),["CAManBase","LandVehicles"]] call mpsf_fnc_getNearbyPlayers) > 0 } do {
		...
		if( count (_nearByUnits) > 0 ) then {
			...
			["player == leader group player",[_percentAlive - 0.15] ] call mpsf_fnc_progressBar;
		};
	};
