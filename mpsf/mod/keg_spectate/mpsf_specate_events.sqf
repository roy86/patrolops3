...
switch (_type) do {
	...
	case "MapClick": {
		_dummy = mpsf_reference_heliempty createVehicleLocal [_param select 0, _param select 1, 0];
		_closest = 5000;
		_id = -1;
		_idxx = 0;
		{
	...
	case "UnitFired": {
		...
		if (ctrlVisible KCMAPFULL) then {
			...
			_m2 = createMarkerLocal ["mil_dot", getPosASL _o];
			...
			if (_type in ["shotMissile", "shotRocket", "shotShell", "shotTimeBomb", "shotPipeBomb", "shotMine", "shotSmoke"]) then {
				_opos = getPosASL _o;
				_m = createMarkerLocal ["mil_dot", [(_opos select 0) + (sin _dir) * _len, (_opos select 1) + (cos _dir) * _len, 0]];
