...
if (isNil "KEGs_FuncsInited") then {
	...
	call compile preprocessFileLineNumbers (mpsf_path_modules+"keg_spectate\mpsf_spect_funcs.sqf");
	spectate_events = compile preprocessFileLineNumbers (mpsf_path_modules+"keg_spectate\mpsf_specate_events.sqf");
	KEGs_menucamslbchange = compile preprocessFileLineNumbers (mpsf_path_modules+"keg_spectate\mpsf_menucamslbchange.sqf");
};
...
if (isNil "KEGs_can_exit_spectator") then {KEGs_can_exit_spectator = false};		// KEGs_can_exit_spectator = true, the player can exit spectating with the X key
if (isNil "KEGs_playable_only") then {KEGs_playable_only = false};			// KEGs_playable_only = true, only playable (MP) or switchableUnits (SP) units will be shown in the target listbox
if (isNil "KEGs_no_butterfly_mode") then { KEGs_no_butterfly_mode = true };		// KEGs_no_butterfly_mode = true, if a player presses Esc he won't switch to butterfly mode but the spectating dialog gets restarted after a second
if (isNil "KEGs_NoMarkersUpdates") then {KEGs_NoMarkersUpdates = false};		// markers are enabled as default, set KEGs_NoMarkersUpdates to true to completely disable marker updates
if (isNil "KEGs_UseLog") then {KEGs_UseLog = false};					// if KEGs_UseLog is set to true, spctator messages are written to group chat and RPT in Singleplayer killed messages are logged too
if (isNil "KEGs_CheckUncon") then {KEGs_CheckUncon = false};				// do also check if a unit is unconscious and don't add it to the target list, KEGs_CheckUncon = true
if (isNil "KEGs_CheckDist") then {KEGs_CheckDist = -1};					// display only units in the target list that are max KEGs_CheckDist meters away, -1 = no check
if (isNil "KEGs_RemoveDeadFilter") then {KEGs_RemoveDeadFilter = true};			// if KEGs_RemoveDeadFilter = true dead units will allways get removed from the target listbox, doesn't matter if you change it during spectating
if (isNil "KEGs_maxDistance") then {KEGs_maxDistance = 50};				// Maximum distance for camera *10m
if (isNil "KEGs_showbuttonattime") then {						// KEGs_showbuttonattime = 300 will show some buttons that you can assign actions to after 5 minutes (300 seconds)
...
_seagull = "noWait";
if (typeName _seagull != "string") then {						// If seagull is valid we came from the respawn scene
...
if (isNil "KEGs_target") then { KEGs_target = objNull };
...
_dummy = mpsf_reference_heliempty createVehicleLocal [0, 0, 0];
...
if (KEGs_no_butterfly_mode) exitWith {
	...
	0 spawn {sleep 0.01; [_player, _killer, "noWait"] execVM (mpsf_path_modules+"keg_spectate\mpsf_specate_Kegetys.sqf");};
};
...
deletevehicle _bird;
KEGs_SPECTATINGON = nil;
[_player, _killer, "noWait"] execVM execVM (mpsf_path_modules+"keg_spectate\mpsf_specate_Kegetys.sqf");