...
if(isNil "mpsf_text_tactical_display") then { mpsf_text_tactical_display = false };
...
if(!mpsf_text_tactical_display) then {
	mpsf_text_tactical_display = true;

	_script = [_row0,_row1,_row2] spawn BIS_fnc_infoText;

	waitUntil{scriptDone _script};

	mpsf_text_tactical_display = false;
};