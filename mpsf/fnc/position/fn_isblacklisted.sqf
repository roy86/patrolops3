...
_shape = _area call mpsf_fnc_getMarkerShape;

_return	= false;
if (_shape == "ICON") then {
	if([_pos,_center] call mpsf_fnc_isSamePosition) then { _return = true };
}else{
	if(_shape in ["RECTANGLE","SQUARE"]) then {
		private ["_corners"];

		_corners = _area call mpsf_fnc_getMarkerCorners;
		...
		if(_dir % 90 != 0) then {
			...
			for "_i" from 0 to (count _corners - 1) do {
				...
				_posNew = [[_shiftedX,_shiftedY],_dir] call mpsf_fnc_rotatePosition;
		...
		_return = [_pos,_corners] call mpsf_fnc_isInRectangle;
	}else{
		if(_shape == "CIRCLE") then {
			_return = [_pos,_area] call mpsf_fnc_isInCircle;
		}else{
			_return = [_pos,_area] call mpsf_fnc_isInEllipse;
		};
	};
};
