...
if (_shape == "ICON") then {
	if( !(_water) && surfaceIsWater _pos ) then {
		...
		_pos = if(_dst != 0) then {
			[_pos,_dst,_dir] call mpsf_fnc_getPosRadius
		}else{
			_pos
		};
	};
	...
	for [{_i = 0}, {_i < 1000 && !_exit}, {_i = _i + 1}] do {
		if (_shape in ["SQUARE","RECTANGLE"]) then {
			_pos = _area call mpsf_fnc_getPosFromRectangle
		}else{
			_pos = _area call mpsf_fnc_getPosFromEllipse
		};
		...
		if(count _blist > 0 && _exit) then {
			{
				if ([_pos,_x] call mpsf_fnc_isBlacklisted) exitwith { _exit = false };
			}foreach _blist;
		};
	};
};
