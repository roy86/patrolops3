scriptName "mpsf\fnc\ai\fn_group_patrol.sqf";
/*
	...
	Example:
		["Marker1",_GROUP,100] spawn mpsf_fnc_group_patrol;
...
if(isNil "_grp") exitWith { ["fnc_Group_Patrol","Undefined Group"] call mpsf_fnc_log };
	...
	if( behaviour leader _grp != "COMBAT" && isNil "_wp") then {
...

		{
			...
			if(mpsf_debug) then {
				...
				["fnc_Group_Patrol",format["Group %1 Assigned WP%2",_grp,_wp]] call mpsf_fnc_log
			};
		} foreach _wparray;
