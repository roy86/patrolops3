...
if(count _staticTypes <= 0 || _numberOfPos <= 0 ) exitWith { ["Static Defense Array Empty"] call mpsf_fnc_hint };
...
for "_i" from 1 to _numberOfPos	do {
	...
	if( count(_pos nearRoads 7) > 0) then {
		_road = (_pos nearRoads 7) select 0;
...
_statics = [];
{
	...
	_veh = if(!isNil "_side") then {
		([ _x select 0, _staticTypes call mpsf_fnc_getArrayRandom, _x select 1, 0, _side] call mpsf_fnc_vehicleSpawner) select 0;
	}else{
		([ _x select 0, _staticTypes call mpsf_fnc_getArrayRandom, _x select 1, 0] call mpsf_fnc_vehicleSpawner) select 0;
	};
	_statics set [count _statics,_veh];

	if(mpsf_debug) then {
		_marker = createMarkerlocal [format["mpsf_temp_static%1",_x select 1],_x select 0];
		_marker setMarkerTypeLocal (_veh call mpsf_fnc_getObjectMarkerType);
		_marker setMarkerColorLocal (_veh call mpsf_fnc_getUnitMarkerColour);
