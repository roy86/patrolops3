...
while {true} do{
	{
		_data = _x getVariable ["mpsf_respawn",nil];

		if(!isNil "_data") then {

			_vehicle = _x;

			_objdata = _vehicle getVariable ["mpsf_object_data",nil];

			_pos	= if(!isNil "_objdata") then { _objdata select 0 }else{ _vehicle call mpsf_fnc_getPos };
			...
			_empty = if( {alive _vehicle} count (crew _vehicle) == 0 && _pos distance (_vehicle call mpsf_fnc_getPos) > 10 ) then { _empty }else{ time };
			_abandoned = if( time - _empty > _timeA ) then { true }else{ false };

			// Get MPSF Variables
			_isMHQ = _vehicle getVariable ["mpsf_mhq",nil];
			_isLCH = _vehicle getVariable ["mpsf_liftchopper",nil];
			_isLFT = _vehicle getVariable ["mpsf_liftable",nil];
			_isRSP = _vehicle getVariable ["mpsf_resupply",nil];

			// MHQ can not be abandoned
			if(!isNil "_isMHQ") then { _abandoned = false };

			if(_destroyed || _abandoned) then {
				_vehicle setVariable ["mpsf_respawn",nil,false];
				_vehicle setVariable ["mpsf_object_data",nil,false];

				if(mpsf_param_removeVehicleWrecks) then { deleteVehicle _vehicle; sleep 5; };
				...
				if( _countR < _limR || _limR < 0 ) then {
					...
					if(!isNil "_isMHQ") then { _vehicle setVariable ["mpsf_mhq",_isMHQ,true] };
					if(!isNil "_isLCH") then { _vehicle setVariable ["mpsf_liftchopper",_isLCH,true] };
					if(!isNil "_isLFT") then { _vehicle setVariable ["mpsf_liftable",_isLFT,true] };
					if(!isNil "_isRSP") then { _vehicle setVariable ["mpsf_resupply",_isRSP,true] };
				};
			};

			if(!isNil "_vehicle") then {
				_vehicle setVariable ["mpsf_respawn",[_timeD,_timeA,_limR],false];
				_vehicle setVariable ["mpsf_object_data",[_pos,_dir,_class,_name,_dead,_empty,_countR],false];
			};
		};
	}forEach vehicles;
