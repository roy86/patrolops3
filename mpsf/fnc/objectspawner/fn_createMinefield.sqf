...
_area		= if( typename (_this select 0) == "STRING") then { (_this select 0) call mpsf_fnc_getmarkershape }else{ "POS" };
...
if!(toUpper _area IN ["SQUARE","RECTANGLE","CIRCLE","ELLIPSE"] ) then {
	...
	_location = createMarkerLocal ["TEMP_MINEAREA",_location call mpsf_fnc_getPos];
...
for "_i" from 1 to _minecount do {
	_pos = _location call mpsf_fnc_getPos;
	if (surfaceIsWater _pos) then {_pos = [_pos select 0, _pos select 1, getTerrainHeightASL _pos]};

	_newmine = createMine [_mineclass call mpsf_fnc_getArrayRandom, _pos, [], 0];
