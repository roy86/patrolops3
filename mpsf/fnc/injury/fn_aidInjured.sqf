...
_damage = (((_unitInAgony getVariable ["mpsf_injury_healedDamage",0]) max 1) min 1.5) * 10; // A MIN of 10secs and MAX of 15secs
...
[_targetTime] spawn {
	...
	if (_targetTime > time) exitWith {mpsf_VAR_HealingStatus = false; mpsf_VAR_animCheck = false};
};
...
if( _firstAider distance _unitInAgony >= 2.5 ) exitWith { _unitInAgony setVariable ["mpsf_injury_healer",objNull,true] };
// If the injured died
...
if(_targetTime <= time) then {
	...
	_healed = switch (true) do {
		...
		case ( (_isMedic) && (_firstAider call mpsf_fnc_hasFirstAidKit) ) : { _unitInAgony spawn mpsf_fnc_useFirstAidKit; 0.025 };
		case ( !(_isMedic) && (_firstAider call mpsf_fnc_hasFirstAidKit) ) : { _unitInAgony spawn mpsf_fnc_useFirstAidKit; 0.05 };
		//if injured does not have ifak then use healers ifak
		case ( (_isMedic) && (_firstAider call mpsf_fnc_hasFirstAidKit) ) : { _firstAider spawn mpsf_fnc_useFirstAidKit; 0.025 };
		case ( !(_isMedic) && (_firstAider call mpsf_fnc_hasFirstAidKit) ) : { _firstAider spawn mpsf_fnc_useFirstAidKit; 0.05 };
		default { 0.07 };
	};
	...
	[ [_firstAider,_unitInAgony],["First Aid Cancelled",0]] call mpsf_fnc_progressBar;
	_unitInAgony setVariable ["mpsf_injury_BeingHealed",false,true];
	_unitInAgony setVariable ["mpsf_injury_healer",objNull,true];
//	hint format ["First Aid Cancelled! time %1 alive healer %2 alive injured %3 dist %4 healinagony %5 healing status %6 vel %7", _time >= time, alive _firstAider, alive _unitInAgony, (_firstAider distance _unitInAgony) < (_curDist + 0.5), !(_firstAider getVariable ["mpsf_injury_inAgony",false]), (mpsf_VAR_HealingStatus), ((animationState player == "ainvpknlmstpsnonwrfldnon_medic0s") || (animationState player == "ainvpknlmstpsnonwrfldnon_medic") || (animationState player == "ainvpknlmstpsnonwrfldnon_medicup") || (animationState player == "ainvpknlmstpsnonwrfldnon_medicup0s") || (animationState player == "amovpknlmstpslowwrfldnon_amovpknlmstpsraswrfldnon"))];
	if(_firstAider == player) then { false call mpsf_fnc_setActionBusy; };
