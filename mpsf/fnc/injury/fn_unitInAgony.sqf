...
if(_unitInAgony getVariable ["mpsf_injury_inAgony",false]) exitWith {};
...
// Begin Bloodloss FSM
[_unitInAgony] spawn mpsf_fnc_injuredBloodLoss;

// Broadcast Damage LOCAL Variable as new PUBLIC Varaible
_unitInAgony setVariable ["mpsf_injury_healedDamage", _unitInAgony getVariable ["mpsf_injury_totalDamage",0],true];
_unitInAgony setVariable ["mpsf_injury_inAgony",true,true];

// Broadcast Side Message of player is in Agony
[_side,"SIDE",format[localize "STR_MPSF_INJURY_ISDOWN",name _unitInAgony,mapGridPosition _unitInAgony] ] call mpsf_fnc_sendChat;
...
waitUntil{
	...
	if(vehicle _unitInAgony != _unitInAgony) then {
		...
		sleep 2;
		_unitInAgony setVelocity [0,0,0];
		[true,"HINTS","You are too injured to be transported"] call mpsf_fnc_hint;
		[_unitInAgony,"Injured_AgonyStart"] call mpsf_fnc_animateUnit;
	};
...
// Reset Action state
if(_unitInAgony == player) then { false call mpsf_fnc_setActionBusy; };
// If alive, Animate then set their damage
if(alive _unitInAgony) then {
	[_unitInAgony,"Injured_AgonyEnd"] call mpsf_fnc_animateUnit;

	_unitInAgony setVariable ["mpsf_injury_totalDamage",_unitInAgony getVariable ["mpsf_injury_healedDamage",0],false];
	_unitInAgony setDamage (_unitInAgony getVariable ["mpsf_injury_healedDamage",0]);

	if !(isNull (_unitInAgony getVariable ["mpsf_injury_healer",objNull])) then {
		[_group,"SIDE",format[localize "STR_MPSF_INJURY_ISUP1",name _unitInAgony,name (_unitInAgony getVariable ["mpsf_injury_healer",objNull])] ] call mpsf_fnc_sendChat;
	}else{
		[_group,"SIDE",format[localize "STR_MPSF_INJURY_ISUP2",name _unitInAgony] ] call mpsf_fnc_sendChat;
	};
}else{
	[_group,"GROUP",format[localize "STR_MPSF_INJURY_ISNOTUP",name _unitInAgony] ] call mpsf_fnc_sendChat;
};
// Resert Unit Variables
_unitInAgony setVariable ["mpsf_injury_dragger",objNull,true];
_unitInAgony setVariable ["mpsf_injury_healer",objNull,true];
