...
if mpsf_debug then { diag_log format ["mpsf_Taskmaster> addTask: %1, %2, %3, %4",_name,_marker,_state,_dest]};

{
	if ( [_x,(_this select 3)] call mpsf_fnc_checkObjCondition ) then {
		...
		switch (toupper(typename _dest)) do {
			...
			case "STRING": { _handle setsimpletaskdestination (_dest call mpsf_fnc_getPos) };
			case "ARRAY": { _handle setsimpletaskdestination (_dest call mpsf_fnc_getPos) };
		};
		...
		if (_x == player) then {
			...
			if (count _marker > 0) then {
				if !(_state in ["succeeded","failed","canceled"]) then {
					...
					{
						_m = createmarkerlocal [(_x select 0),(_x select 1) call mpsf_fnc_getPos];
...
mpsf_tasks_TasksLocal set [count mpsf_tasks_TasksLocal,[_name,_state,_handles]];