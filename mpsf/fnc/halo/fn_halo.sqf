...
_position	= [_this select 1,25,round random 360, true] call mpsf_fnc_getPos;
...
if(_unit == player) then { true call mpsf_fnc_setActionBusy; };

[_unit] spawn mpsf_fnc_haloLoadout;
[_unit] spawn mpsf_fnc_haloChemlights;
[_unit] spawn mpsf_fnc_haloEffects;
[_unit] spawn mpsF_fnc_haloAutoOpen;
...
if(_unit == player) then { false call mpsf_fnc_setActionBusy; };
