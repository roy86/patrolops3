...
_uavClass    = (mpsf_CfgSupport_UASVehicles select 1) select ([WEST, EAST, RESISTANCE] find (side _UAVOp));
_uavTerminal = (mpsf_CfgSupport_UASVehicles select 0) select ([WEST, EAST, RESISTANCE] find (side _UAVOp));
_position = [_UAVOp,3000,180] call mpsf_fnc_getPos;
_position set [2,500];
...
mpsf_support_ActiveUAV = _uav; publicVariable "mpsf_support_ActiveUAV";
