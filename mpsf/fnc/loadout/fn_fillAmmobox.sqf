...
_cache setVariable ["mpsf_VAS",true,false];
if(mpsf_ammobox_vas_enabled) exitWith {};

player reveal _cache;

while {alive _cache} do {

	_data = call mpsf_fnc_getCfgMpsfLoadout;

	if(isNil "_data") exitWith{};

	_class = player getVariable ["mpsf_VAR_roleAttribute","Rifleman"];
	_rank = rank player;
	_timer = 30;
...
//	_magazinecapacity  = 300 max ( (getNumber (configFile >> "CfgVehicles" >> typeOf _cache >> "transportMaxMagazines")) min 1400);
	...
	waitUntil { sleep 10; _timer = _timer - 1; !(rank player == _rank && _timer > 0 && _class == player getVariable ["mpsf_VAR_roleAttribute","Rifleman"]) };
};
