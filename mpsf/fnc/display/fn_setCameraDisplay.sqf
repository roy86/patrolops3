...
if (_index in (actionkeys 'nightvision') || _key < 0) then {
	if(isNil "mpsf_VAR_camVision_index") then { mpsf_VAR_camVision_index = 0 };
	mpsf_VAR_camVision_index = mpsf_VAR_camVision_index + 1;
	_vision = mpsf_VAR_camVision_index % 9;
	switch (_vision) do {
		case 0: {
			camusenvg false;
