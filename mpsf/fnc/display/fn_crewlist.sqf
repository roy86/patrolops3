...
uiNamespace setVariable ["mpsf_hud_crewlist", objnull];

if(isnull (uiNamespace getVariable "mpsf_hud_crewlist")) then { cutrsc ["mpsf_hud_crewlist", "PLAIN"];};
_ctrl = (uiNamespace getVariable 'mpsf_hud_crewlist') displayCtrl 860009;

while{isNil "mps_mission_finished"} do{

	_text = "";
	if( mpsf_hud_screen_display && !visibleMap && isNil("BIS_DEBUG_CAM") ) then {
		...
		if(vehicle player != player) then {
			...
			_text = _text + format ["<t>%1</t><br/>", _name];
			{
				if(_x == driver _vehicle || _x == commander _vehicle) then {
					_icon = [typeof vehicle _x] call mpsf_fnc_getCfgIcon;
