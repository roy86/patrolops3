...
if(_turretcount > 0) then {
...

	{
		...
		[true,"HINTS",format[localize "STR_MPSF_VEHRSUP_REARM",_vehname,_magname]] call mpsf_fnc_hint;
		sleep 5;
	...
	if(!alive player || !alive _Vehicle || _vehicle distance _position > 3) exitWith { [true,"HINT",format[localize "STR_MPSF_VEHRSUP_STOP",_vehname]] call mpsf_fnc_hint };
