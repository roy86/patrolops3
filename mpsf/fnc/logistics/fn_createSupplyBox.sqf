...
_ammobox setVariable ["mpsf_loadout_boxID",[_side,_boxid],true];
...
[_ammobox] spawn mpsf_fnc_setAsDraggable;
[_ammobox] spawn mpsf_fnc_setDamageEH_C4Only;

[_side,_boxid,_ammobox] call mpsf_fnc_declareSupplybox;

[_ammobox,_side] spawn {
	_box = _this select 0;
	_side = _this select 1;
	waitUntil{ sleep 10; count ([_box,1000,_side,["CAManBase","LandVehicle","Air"]] call mpsf_fnc_getNearbyPlayers) == 0 };
	[_side,"SIDE",format["Supplybox GRID:%1 has been recovered",mapGridPosition _box]] call mpsf_fnc_sendChat;
	deleteVehicle _box;
};

[player,"HINT","You have created a supplybox<br/><br/>You can transport the supply box by loading it onto a truck or using a helicopter to slingload it to its destination.<br/><br/>It will be recovered as soon as nearby players leave its vicinity."] call mpsf_fnc_hint;