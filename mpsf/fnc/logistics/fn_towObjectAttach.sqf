...
_loadable = ["Towable",_object,_towVeh] call mpsf_fnc_logisticsCheck;
if !(_loadable) exitWith { [true,"HINT",format[ localize "STR_MPSF_LOGIST_TOOHEAVY",[_object] call mpsf_fnc_getCfgText,[_towVeh] call mpsf_fnc_getCfgText] ] call mpsf_fnc_hint; false };
...
_towVehSize = _towVeh call mpsf_fnc_boundingBoxLWH;
_objectSize = _object call mpsf_fnc_boundingBoxLWH;
...
_towVeh setVariable ["mpsf_isTowingObj",_object,false];
_object setVariable ["mpsf_isTowingObj",_towVeh,false];
_action = _towVeh addAction [ format["<t color='#ffc600'>Unhook %1</t>",[_object] call mpsf_fnc_getCfgText],{ [(vehicle player)] spawn mpsf_fnc_towObjectRelease; },[],100,false,true,"","!isNull (_target getVariable [""mpsf_isTowingObj"",objNull]) && speed _target < 1"];
...
[_towVeh,_object] spawn mpsf_fnc_towRope;
...
_towImpVelocity = false;
	...
	_wheelPpos = (_objWheel) call mpsf_fnc_getPos;
