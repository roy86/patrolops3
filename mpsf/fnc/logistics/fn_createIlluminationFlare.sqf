/*
	...
	Syntax:
	[<position>,<type>] call mpsf_fnc_craeteFlare
...
_type = if( typeName (_this select 0) == typeName objNull) then {
	(_this select 0) getVariable ["mpsf_VehicleCfg_IllumClass",(_this select 1)];
}else{
	(_this select 1);
};
...
if(isNil "_class") exitWith { ["Flare","Incorrect Class Type Requested",true] call mpsf_fnc_log };

_flare = createVehicle [_class,_position,[],0,"FLY"];
_flare setVelocity [0,0,-0.01];

[true,"HINT",format[localize "STR_MPSF_LOGIST_ILLUMFLARE0",_class,mapGridPosition _position] ] call mpsf_fnc_hint;
["Flare",format["%1 Flare created at %2",_class,_position] ] call mpsf_fnc_log;
